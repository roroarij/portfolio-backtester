...
export async function fetchTickerHistory(symbol, range) {
...
export async function buildPortfolioHistory(rawHoldings, range) {
  ...
  for (const date of allDates) {
    ...
    const value = holdings.reduce((total, holding) => {
      const close = lastSeen.get(holding.symbol) || 0;
      const holdingValue = Number((holding.quantity * close).toFixed(2));
