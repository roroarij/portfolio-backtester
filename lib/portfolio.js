...
export async function buildPortfolioHistory(rawHoldings, range) {
  ...
  const startDate = uniqueSymbols
    .map((symbol) => historyMap.get(symbol)[0]?.date)
    .sort()
    .at(-1);
  ...
  const allDates = [...new Set(uniqueSymbols.flatMap((symbol) => historyMap.get(symbol).map((point) => point.date)))]
  ...
  for (const date of allDates) {
    ...
    if (lastSeen.size !== uniqueSymbols.length) {
      continue;
    }

    const value = holdings.reduce((total, holding) => {
      return total + holding.quantity * lastSeen.get(holding.symbol);
    }, 0);
